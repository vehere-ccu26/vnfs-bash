...
ALTADDRESSES=${IPLIST}
CA_ORG='/O=vehere.com/OU=generate-CA/emailAddress=info@vehere.com'
CA_DN="/CN=Vehere CISUI${CA_ORG}"
CACERT=${DIR}/ca
SERVER="${DIR}/${host}"
SERVER_DN="/CN=${host}$CA_ORG"
keybits=2048
openssl=$(which openssl)
KIBANAUSER=${KIBANAUSER:=$USER}
...
if [ $kind == 'server' ]; then
	...
	if [ -f $SERVER.csr -a ! -f $SERVER.crt ]; then
		...
		%%% 
		%%% [notice]
		%%% explicitText            = "This CA is for a local Kibana server installation only"
		%%% organization            = "Vehere"
		%%% noticeNumbers           = 1
